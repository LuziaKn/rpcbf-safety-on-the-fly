...
from og.dyn_types import HFloat, State, Disturb


def hocbf(h_fn: Callable, f_fn, alpha0: float | HFloat, state: State, dstb: Disturb) -> HFloat:

    h_h = h_fn(state)
    hx_hx = jax.jacobian(h_fn)(state)
    nh, nx = hx_hx.shape
    assert h_h.shape == (nh,)
