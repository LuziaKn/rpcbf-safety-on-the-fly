...
class PCBFRolloutJax:
    ...
    def solve_cbf_qp(
        ...
        settings = JaxProxQP.Settings.default_float32()
        problem = JaxProxQP.QPModel.create(H, g, C, ub, l_box, u_box)
    ...
    def rollout(self, key_base: PRNGKeyArray, cbf_pol: Policy, nom_pol: Policy, x0: State, rollout_T: int):
        def body(carry, kk):
            x_, sampler, pbar_state = carry
            # Compute value function.
            sampler, hT_dstb, h_Vh, grad_h_Vh, h_f, h_G, info_sampler = sampler.get_value_and_grad(cbf_pol, x_)

            # Solve CBF-QP
            u_nom = nom_pol(x_)
            u_cbf, info_qp = self.solve_cbf_qp(u_nom, h_Vh, grad_h_Vh, h_f, h_G)
            ...
            return (x_new, sampler, pbar_state), (x_new, u_nom, u_cbf, dstb_env, hT_dstb, info)

        progress_meter = diffrax.TextProgressMeter()
        pbar_state0 = progress_meter.init()
        T_kk = jnp.arange(rollout_T)
        carry_init = (x0, self.sampler, pbar_state0)
        (_, self.sampler, pbar_state_final), (T_xnew, T_u_nom, T_u_cbf, T_dstb_env, ThH_dstb, T_info) = lax.scan(
        ...
        return Tp1_x, T_u_cbf, T_u_nom, Tp1h_h, T_dstb_env, ThH_dstb, T_info
