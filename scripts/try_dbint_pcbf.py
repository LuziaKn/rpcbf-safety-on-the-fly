...
def main():
    ...
    rollout_T = 64  # horizon evaluation rollout

    logger.info("horizon: {}, rollout_T: {}".format(horizon, rollout_T))

    rngs = nnx.Rngs(12345)

    task = DoubleIntWall()

    sampler_cfg = ZeroSampler.Cfg()
    sampler = ZeroSampler.create(rngs(), task, horizon, sampler_cfg)
    ...
    bTp1_x_cbf, _, _, bT_Vh_max_cbf, bT_dstb_env, bT_dstb, bH_info = jax_jit_np(jax_vmap(rollout_fn))(b_keys)
    ...
    fig_path = plot_dir / "pcbf_contour.pdf"
